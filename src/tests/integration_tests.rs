...
fn test_uppercase_extension() {
    let temp_dir = TempDir::new().unwrap();
    let file = create_test_file(&temp_dir, "TEST.PY", "print('Uppercase!')");
...
fn test_multiple_dots_in_filename() {
    let temp_dir = TempDir::new().unwrap();
    let file = create_test_file(&temp_dir, "test.backup.py", "print('Multiple dots!')");
    ...
    Command::cargo_bin("code-runner")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Multiple dots!"));
}